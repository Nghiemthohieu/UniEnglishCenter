 ...
 */
export async function addWordCalendar(work_calendar: CreateWordCalendar[]): Promise<CreateWordCalendar[]> {

  // Gửi dữ liệu tới API dưới dạng JSON
  const res = await fetch(`${baseUrl}/work_calendar/alls`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json", // Đặt Content-Type là application/json
    },
    body: JSON.stringify(work_calendar), // Chuyển đối tượng payload thành chuỗi JSON
  });
