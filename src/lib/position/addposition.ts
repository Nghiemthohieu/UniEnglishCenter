 ...
 */
export async function addPosition(position: PositionCreated): Promise<PositionCreated> {
  ...
  const res = await fetch(`${baseUrl}/position`, {
    ...
    body: JSON.stringify(position), // Chuyển đối tượng payload thành chuỗi JSON
  });
  ...
  const data = (await res.json()) as PositionCreated;
  return data;
}
