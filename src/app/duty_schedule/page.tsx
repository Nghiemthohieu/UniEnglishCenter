...
import { fetchDutyScheduleByOffice } from "@/lib/dutyschedule/getdutyschedulebyhuman"
import AddDutyScheduleForm from "@/components/addForm/adddutyschedulefofefform"
import UpdateDataDutySchedule from "@/components/updateForm/update_data_dutyschedule"
import TableDataDutySchedule from "@/components/table/table_data_dutyschedule"
...
export default function Dutyschedule() {
  ...
  // ✅ Fetch sự kiện
  useEffect(() => {
    fetch(`${baseUrl}/duty_schudule/office/2`)
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        const { data } = await fetchDutyScheduleByOffice()
  ...
  return (
    <div className="p-[10px] relative">
      ...
      {LoadCalendar ? (
        ...
        <TableDataDutySchedule data={WordCalendar} onClose={onclickaddWordCalendarExit} updateOnClick={handleUpdateWordCalendarClick}/>
      )}

      {addWordCalendarExit && (
        <AddDutyScheduleForm onClose={onclickaddWordCalendarExit} shift={shifts}/>
      )}

      {
        updateWordCalendarExit && (
          <UpdateDataDutySchedule wordcalendar={SelectedWordCalendar} onClose={onclickUpdateWordCalendarExit} shift={shifts}/>
        )
      }
      {loadShift ? (
        <div className="mx-auto h-full w-full max-w-sm rounded-md border border-blue-300 p-4">
