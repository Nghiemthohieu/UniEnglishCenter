...
import { updateCourses } from "@/lib/courses/updatecourses"

type Props = {
  onClose: () => void;
  course: Courses | null;
}
export default function UpdateDataCustomerSources({onClose, course}: Props) {
  const [formData, setFormData] = React.useState<Courses | null>(null)
  React.useEffect(() => {
        if (course) {
            setFormData(course);
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateCourses(coursesData); // Gửi dữ liệu nhân sự và ảnh CMND
