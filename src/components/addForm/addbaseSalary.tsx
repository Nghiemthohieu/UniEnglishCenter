...
import { addBaseSalary } from "@/lib/baseSalary/addbasesalary"
...
export default function AddBaseForm({ onClose, position }: Props) {
  ...
  const handleSubmitUpdate = async () => {
    ...
    try {
      const response = await addBaseSalary(salaryData);
  ...
  return (
    <div className="fixed top-0 left-0 bottom-0 right-0 z-99 w-full h-full flex justify-center items-center addPerson">
        ...
        <div className="fixed z-999 min-w-[400px]">
            <Card>
            <CardHeader>
                <CardTitle className="uppercase drop-shadow-md text-[16px]">Thêm mốc phần trăm ds đội</CardTitle>
            </CardHeader>
