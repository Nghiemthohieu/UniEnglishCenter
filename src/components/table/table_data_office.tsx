...
import { deleteOfficeById } from "@/lib/office/deleteoffice"

type Props = {
  data: Office[];
  onClick: () => void;
  updateOnClick: (office: Office) => void;
}

export default function TableDataOffice({ data, onClick, updateOnClick}: Props) {
  ...
  const columns: ColumnDef<Office>[] = [
    ...
    { accessorKey: "acronym", header: "Viết tắt", enableHiding: false },
    {
      id: "actions",
      header: "Thao tác",
      enableHiding: false,
  ...
  const deleteOnClick = async (officeid: Office) => {
        // Hiển thị cảnh báo xác nhận trước khi xóa
        const confirmDelete = window.confirm("Bạn có chắc chắn muốn xóa văn phòng này không?");
        ...
        try {
          const success = await deleteOfficeById(officeid.ID);
          if (success) {
            console.log("chức vụ đã được xóa thành công.");
            // Cập nhật lại dữ liệu sau khi xóa thành công
          } else {
            console.log("Xóa chức vụ không thành công.");
          }
        } catch (error) {
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.filter((employee) => {
      return (
        ...
        String(employee.acronym || "").toLowerCase().includes(keyword)
      )
    })
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách văn phòng</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex items-center justify-end space-x-2 pt-4">
          <div className="flex-1 text-sm text-muted-foreground">
            ...
            {table.getFilteredRowModel().rows.length} văn phòng.
          </div>
