...
import { deleteCustomerSourcesById } from "@/lib/customer_sources/deletecustomersources"

type Props = {
  data: CustomerSource[];
  onClick: () => void;
  updateOnClick: (customer_source: CustomerSource) => void;
}

export default function TableDataCustomerSources({ data, onClick, updateOnClick}: Props) {
  const [sorting, setSorting] = React.useState<SortingState>([])

  // Di chuyển khai báo columns vào đây
  const columns: ColumnDef<CustomerSource>[] = [
    { accessorKey: "ID", header: "ID", enableHiding: false },
    { accessorKey: "name", header: "Chức vụ", enableHiding: false },
    {
      id: "actions",
      header: "Thao tác",
      enableHiding: false,
  ...
  const deleteOnClick = async (CustomerSourceid: CustomerSource) => {
      // Hiển thị cảnh báo xác nhận trước khi xóa
      const confirmDelete = window.confirm("Bạn có chắc chắn muốn xóa nguồn khách hàng này không?");
      ...
      try {
        const success = await deleteCustomerSourcesById(CustomerSourceid.ID);
        if (success) {
          console.log("nguồn khách hàng đã được xóa thành công.");
          // Cập nhật lại dữ liệu sau khi xóa thành công
        } else {
          console.log("Xóa nguồn khách hàng không thành công.");
        }
      } catch (error) {
        console.error("Lỗi khi xóa nguồn khách hàng:", error);
      }
    }; 
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách nguồn khách hàng</CardTitle>
      </CardHeader>
      <CardContent>
