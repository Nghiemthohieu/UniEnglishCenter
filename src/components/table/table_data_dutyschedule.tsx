...
import { WordCalendar } from "@/JSON/wordcalendar"
import { deleteWordCalendarById } from "@/lib/wordcalendat/deletewordcalendar"
import { deleteDutyScheduleById } from "@/lib/dutyschedule/deletedutyschedule"
...
export default function TableDataDutySchedule({ data ,onClose, updateOnClick}: Props) {
  ...
  const deleteOnClick = async (WordCalendarid: WordCalendar) => {
      ...
      try {
        const success = await deleteDutyScheduleById(WordCalendarid.ID);
        if (success) {
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách đăng ký lịch trực</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex items-center justify-end space-x-2 pt-4">
          <div className="flex-1 text-sm text-muted-foreground">
            ...
            {table.getFilteredRowModel().rows.length} lịch trực.
          </div>
