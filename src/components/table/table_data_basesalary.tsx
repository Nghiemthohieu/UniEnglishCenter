...
export default function TableDataBaseSalary({ data, onClick, updateOnClick}: Props) {
  ...
  const deleteOnClick = async (salary: softSalary) => {
      ...
      try {
        const success = await deleteBaseSalaryById(salary.ID);
        if (success) {
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách phần trăm ds Đội</CardTitle>
      </CardHeader>
      <CardContent>
