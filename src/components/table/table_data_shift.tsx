...
import { deleteShiftById } from "@/lib/shift/delete.shift"

type Props = {
  data: Shift[];
  onClick: () => void;
  updateOnClick: (shift: Shift) => void;
}

export default function TableDataShift({ data, onClick, updateOnClick}: Props) {
  const [sorting, setSorting] = React.useState<SortingState>([])

  // Di chuyển khai báo columns vào đây
  const columns: ColumnDef<Shift>[] = [
    { accessorKey: "id", header: "ID", enableHiding: false },
    { accessorKey: "shift", header: "Ca làm", enableHiding: false },
    { accessorKey: "time_start", header: "Thời gian bắt đầu", enableHiding: false },
    { accessorKey: "time_out", header: "Thời gian kết thúc", enableHiding: false },
    {
      id: "actions",
      header: "Thao tác",
      enableHiding: false,
  ...
  const deleteOnClick = async (shiftid: Shift) => {
      ...
      try {
        const success = await deleteShiftById(shiftid.id);
        if (success) {
          console.log("ca làm việc đã được xóa thành công.");
          // Cập nhật lại dữ liệu sau khi xóa thành công
        } else {
          console.log("Xóa ca làm việc không thành công.");
        }
      } catch (error) {
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.filter((employee) => {
      return (
        ...
        String(employee.shift || "").toLowerCase().includes(keyword) ||
        String(employee.time_out || "").toLowerCase().includes(keyword) ||
        String(employee.time_start || "").toLowerCase().includes(keyword)
      )
    })
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none mt-[20px] mb-[20px]">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách ca làm</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex items-center justify-end space-x-2 pt-4">
          <div className="flex-1 text-sm text-muted-foreground">
            ...
            {table.getFilteredRowModel().rows.length} ca làm việc.
          </div>
