...
import { deletePaymentFormById } from "@/lib/payment_forms/deletepaymentform"

type Props = {
  data: PaymentForm[];
  onClick: () => void;
  updateOnClick: (payment_form: PaymentForm) => void;
}

export default function TableDataPaymentForm({ data, onClick, updateOnClick}: Props) {
  const [sorting, setSorting] = React.useState<SortingState>([])

  // Di chuyển khai báo columns vào đây
  const columns: ColumnDef<PaymentForm>[] = [
  ...
  const deleteOnClick = async (payment_form_id: PaymentForm) => {
      // Hiển thị cảnh báo xác nhận trước khi xóa
      const confirmDelete = window.confirm("Bạn có chắc chắn muốn xóa hình thức thanh toán này không?");
      ...
      try {
        const success = await deletePaymentFormById(payment_form_id.ID);
        if (success) {
          console.log("hình thức thanh toán đã được xóa thành công.");
          // Cập nhật lại dữ liệu sau khi xóa thành công
        } else {
          console.log("Xóa hình thức thanh toán không thành công.");
        }
      } catch (error) {
        console.error("Lỗi khi xóa hình thức thánh toán:", error);
      }
    }; 
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách hình thức thanh toán</CardTitle>
      </CardHeader>
      <CardContent>
