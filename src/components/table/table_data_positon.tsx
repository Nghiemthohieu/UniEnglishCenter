...
type Props = {
  data: Position[];
  onClick: () => void;
  updateOnClick: (position: Position) => void;
}

export default function TableDataPosition({ data, onClick, updateOnClick}: Props) {
  ...
  const deleteOnClick = async (Positionid: Position) => {
      ...
      try {
        const success = await deletePositionById(Positionid.ID);
        if (success) {
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách Chức vụ</CardTitle>
      </CardHeader>
      <CardContent>
