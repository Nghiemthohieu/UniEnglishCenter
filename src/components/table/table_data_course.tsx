...
import { deleteCoursesById } from "@/lib/courses/deleteCourses"

type Props = {
  data: Courses[];
  onClick: () => void;
  updateOnClick: (cource: Courses) => void;
}

export default function TableDataCourse({ data, onClick, updateOnClick}: Props) {
  ...
  const deleteOnClick = async (course_id: Courses) => {
      ...
      try {
        const success = await deleteCoursesById(course_id.ID);
        if (success) {
          console.log("khóa học đã được xóa thành công.");
          // Cập nhật lại dữ liệu sau khi xóa thành công
        } else {
          console.log("Xóa khóa học không thành công.");
        }
      } catch (error) {
  ...
  return (
    <Card className="shadow-[0_2rem_3rem_rgba(132,139,200,0.18)] hover:shadow-none">
      <CardHeader>
        <CardTitle className="text-xl uppercase">Danh sách khóa học</CardTitle>
      </CardHeader>
      <CardContent>
